...
PT:CreateButton({Name="Teleport to Player in Pod",Callback=function() local f=false for _,p in pairs(P:GetPlayers()) do local hrp=p.Character and p.Character:FindFirstChild("HumanoidRootPart") if p.Character and p.Character~=PChar and p.Character:GetAttribute("InPod") and hrp then HRP.CFrame=hrp.CFrame+Vector3.new(0,0,0) f=true break end end if not f then R:Notify({Title="Error",Content="No player was in pod",Duration=3,Image="triangle-alert"}) end end})
