...
TT:CreateButton({Name="Teleport to Beast",Callback=function()
    local bName=(RepS:FindFirstChild("Beast1")and RepS.Beast1.Value)or(RepS:FindFirstChild("Beast2")and RepS.Beast2.Value)
    local beast=nil for _,plr in ipairs(P:GetPlayers())do if plr.Name==bName or plr.DisplayName==bName then beast=plr break end end
    if beast and beast.Character and beast.Character:FindFirstChild("HumanoidRootPart")then HRP.CFrame=beast.Character.HumanoidRootPart.CFrame+Vector3.new(0,5,0)
    else R:Notify({Title="Error",Content="No Beast found",Duration=3,Image="triangle-alert"})end
end})
