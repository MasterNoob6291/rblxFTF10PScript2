...
R:Notify({Title="Success!",Content="FTF Hub Loaded! Use 'K' to toggle UI",Duration=3,Image="check",Actions={}})

-- Player Tab
local PT=W:CreateTab("Player Controls","circle-user")
PT:CreateSection("Basic Controls") PT:CreateDivider()
PT:CreateSlider({Name="WalkSpeed",Range={8,500},Increment=1,Suffix="Speed",CurrentValue=16,Flag="WalkSpeed",Callback=function(v) H.WalkSpeed=v end})
PT:CreateSlider({Name="JumpHeight",Range={5,30},Increment=1,Suffix="Height",CurrentValue=7.2,Flag="JumpHeight",Callback=function(v) H.JumpHeight=v end})
...
PT:CreateButton({Name="Teleport to Incomplete Computer",Callback=function() for _,c in pairs(Map:GetChildren()) do if c:IsA("Model") and c.Name=="ComputerTable" then local s=c:FindFirstChild("Screen") local t=c:FindFirstChild("ComputerTrigger1") if s and t and s.Color~=Color3.fromRGB(60,255,0) then HRP.CFrame=t.CFrame+Vector3.new(0,5,0) break end end end end})
PT:CreateButton({Name="Teleport to Player in Pod",Callback=function() local f=false for _,p in pairs(P:GetPlayers()) do if p.Character and p.Character~=C then local F,POD=p.Character:GetAttribute("Freezing"),p.Character:GetAttribute("InPod") local hrp=p.Character:FindFirstChild("HumanoidRootPart") if F and POD and hrp then HRP.CFrame=hrp.CFrame+Vector3.new(0,5,0) f=true break end end end if not f then R:Notify({Title="Error",Content="No player was in pod",Duration=3,Image="triangle-alert",Actions={}}) end end})

-- Statistics Tab
local ST = W:CreateTab("Statistics","circle-user")
local StatsSection = ST:CreateSection("Game Stats")
local Beast1 = ST:CreateLabel("Beast1: LOADING..", "skull")
local Beast2 = ST:CreateLabel("Beast2: LOADING..", "skull")
local MapLabel = ST:CreateLabel("Map: LOADING..", "map")  -- renamed so it doesn't conflict with Map variable
